...
/* O, o pour jouer sur l'ouverture */
/* P,p pour la perspective         */
/* +,- pour le zoom                */
...
void projection(void) {
  if ( proj ) 
    {/* si nous avons une projection perspective */
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    gluPerspective(60.0*ouverture,(double) ww/hh,0.1,200.0);
    glMatrixMode(GL_MODELVIEW);
    glTranslatef(0.0F,0.0F,-5.0F*zoomPersp);  
    setManipulateurDistance(5.0F*zoomPersp); }
    else {/* si nous avons une projection parallèle */
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    glOrtho(-2.5*zoomParal,2.5*zoomParal,-2.5*zoomParal*(double) hh/ww,
             2.5*zoomParal*(double) hh/ww,-25,25);
    glMatrixMode(GL_MODELVIEW);
...
void display(void) {   
  ...
  /* appel à la fonction qui gère la projection */
  projection();
  /* définition du contenu de la scène */
  glPushMatrix();
  manipulateurSouris();
  manipulateurClavier();
